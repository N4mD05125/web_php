...
$directory = "answer_homework_file/";

echo "<h1> Answer list </h1>";
...
if (is_dir($directory)) {
    ...
    foreach ($files as $file) {
	    if ($file !== '.' && $file !== '..') {
		echo "<h4>" . $file . "</h4>";
		echo '<form action="teacher_download_answer.php" method="post">';
                echo '<button type="submit" name="download" value="' . htmlspecialchars($file) . '">Download</button>';
                echo '</form>';

		echo '<form action="teacher_remove_answer.php" method="post">';
