...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
		...
		echo "<br/>";
            ...
            $stmt = $conn->prepare("INSERT INTO thong_tin (username, password, name, email, sdt) values (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $update_username, $update_password, $update_name, $update_email, $update_sdt);

            if ($stmt->execute()) {
		    echo "Your student information has been added!" . "<br/>";
		    $url = 'http://localhost/teacher_dashboard.php';
        	    $linkText = 'Return to dashboard!';
        	    echo '<a href="' . $url . '">' . $linkText . '</a>';
        	    echo "<br/>";
